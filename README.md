...
1.  **File trình tự Gen (`sequence.fasta` - 41.9 MB):** Truy cập NCBI Nucleotide, tìm kiếm bộ lọc `33175[BioProject] OR 33317[BioProject]` (RefSeq Targeted Loci 16S), sử dụng tính năng **Send to -> File -> Format: FASTA** để tải về. File này chứa chuỗi ADN của 26.870 chủng vi khuẩn.
2.  **File Đại danh bạ (`RefSeq-release235.catalog.gz` - 3.3 GB):** Truy cập máy chủ FTP của NCBI theo đường dẫn `/refseq/release/release-catalog/` để tải file danh bạ đối chiếu ngoại tuyến mới nhất, giúp ánh xạ mã Accession (e.g., `NR_113675.1`) sang mã số phân loại TaxID.
...
Kích hoạt môi trường và bật server FastAPI:
...
Truy cập vào trình duyệt: http://127.0.0.1:8000/docs

Tìm đến API /run-pipeline, bấm Try it out và điền dữ liệu JSON với đường dẫn tuyệt đối chuẩn macOS (Sử dụng dấu gạch chéo xuôi /):
...
```
Bấm Execute. Tiến trình sẽ tự động chạy ngầm (Background Task). Bạn có thể theo dõi tiến độ thời gian thực bằng cách đọc file log tại thư mục kết quả.
